...
import { api, Category, Product, Supplier, getUploadsBase } from '../api/client';
...
export default function CatalogView({
  ...
  useEffect(() => {
    api.getSuppliers().then(setSuppliers).catch(() => undefined);
  }, []);
  ...
  const saveProduct = async () => {
    ...
    fd.append('controlled_substance', form.controlledSubstance ? '1' : '0');
    fd.append('generic_group', form.genericGroup);
    fd.append('reorder_level', form.reorderLevel || '0');
  ...
  const editProduct = (p: Product) => {
    setForm({
      ...
      trackStock: !!p.stock,
      img: p.img || '',
      controlledSubstance: !!p.controlled_substance,
      genericGroup: p.generic_group || '',
      reorderLevel: String(p.reorder_level ?? 0),
      ...
      {tab === 'products' && canProducts && (
        <div className="page-grid">
          <div className="panel" style={{ padding: '1rem' }}>
            ...
            <div className="field">
              <label>Generic group</label>
              <input
                placeholder="e.g. Paracetamol — links brand/generic products for substitution"
                value={form.genericGroup}
                onChange={(e) => setForm({ ...form, genericGroup: e.target.value })}
              />
            </div>
            <div className="field">
            ...
            >
              <input
                ...
                onChange={(e) => setForm({ ...form, controlledSubstance: e.target.checked })}
              />
              Controlled substance (requires a second approver to dispense)
            </label>
            <PhotoPicker
              value={form.img}
                    ...
                    <td>
                      <div>
                        ...
                        {p.controlled_substance && (
                          <span className="stat-pill" style={{ marginLeft: '0.4rem' }}>
                            controlled
                          </span>
                        )}
                      </div>
                      <div className="muted" style={{ fontSize: '0.8rem', marginTop: '0.15rem' }}>
                        ...
                        {p.generic_group ? ` · ${p.generic_group}` : ''}
                      </div>
                    </td>
                    <td>
                      {symbol}
